...
def scrape_steam_reviews(username, all):
    ...
    while True:
        ...
        for review in reviews:
            ...
            review_text = md(review.find("div", class_="content").decode_contents())
            ...
            review_data = {
                ...
                "last_updated": None if not last_updated or (last_updated == review_date) else parser.parse(last_updated)
            }
...
def print_review(review):
    ...
    print(f"Review Date: {review['review_date']} (last updated: {review['last_updated']})")
    print(f"Playtime: {review['total_playtime']} (at review: {review['playtime_at_review']})")
...
def save_review(review):
    ...
    with open(steam_id + '.md', 'w') as f:
        ...
        if review['last_updated']:
            f.write('\n')
            f.write('last_updated: ' + review['last_updated'].strftime("%Y-%m-%d"))
        f.write('\n')
...
def main():
    parser = argparse.ArgumentParser(description="Archive Steam reviews from a specific user account.")
    parser.add_argument("--username", help="Steam username for which to download reviews.")
    parser.add_argument('--all', default=False, action='store_true', help="Download all reviews (or just the first page).")
    parser.add_argument('--save', default=True, action='store_true', help="Save downloaded reviews to filesystem (or print to stdout).")
